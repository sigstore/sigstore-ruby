...
module Sigstore
  ...
  VerificationMaterials = Struct.new(:hashed_input, :certificate, :signature, :offline, :rekor_entry, :input_bytes,
    ...
    def find_rekor_entry(rekor_client)
      ...
      raise "Rekor entry not found" unless entry

      # debug

      actual_body = JSON.parse(entry.body.unpack1("m0"))
      if actual_body != expected_entry.to_h
        raise "Invalid rekor entry: expected #{expected_entry.to_h}, got #{actual_body}"
      end
    ...
    def self.from_bundle(input:, bundle:, offline:)
      ...
      if media_type == BundleType::BUNDLE_0_3
        ...
        raise "Expected chain" if certs.empty?

        leaf_cert = certs.shift
        raise "Expected leaf" unless cert_is_leaf?(leaf_cert)
      ...
      if media_type == BundleType::BUNDLE_0_1
        ...
        raise "must contain an inclusion proof" unless tlog_entry.inclusion_proof
        raise "must contain a checkpoint" unless tlog_entry.inclusion_proof.checkpoint.envelope
      end
    ...
    def self.cert_is_leaf?(cert)
      raise "invalid X.509 version: #{cert.version.inspect}" if cert.version != 2 # v3

      return false if cert_is_ca?(cert)

      key_usage = cert.find_extension("keyUsage") || raise("no keyUsage in #{cert.extensions.map(&:to_h)}")
      digital_signature = key_usage&.value&.include?("Digital Signature") # TODO: proper inclusion checking

      unless digital_signature
        raise "invalid certificate for Sigstore purposes: missing digital signature usage: #{key_usage.to_h}"
      end
    ...
    def self.cert_is_ca?(cert)
      ...
      raise "invalid certificate states: KeyUsage.keyCertSign"
    end
  end
end
